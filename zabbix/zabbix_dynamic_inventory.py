...
class ZabbixInventory(object):
    ...
    def get_list(self, api):
        ...
        data['Available_Zabbix_Agents'] = self.hoststub()
        ...
        for host in hostsData:
            ...
            try:
                hostip = api.hostinterface.get({'hostids': hostid, 'filter': {'type': 1}, 'output': 'extend', 'selectGroups': 'extend'})[0]['ip']
            except:
                hostip = api.hostinterface.get({'hostids': hostid, 'filter': {'type': 2}, 'output': 'extend', 'selectGroups': 'extend'})[0]['ip']

            metadata = {hostname: {'ansible_host': hostip, 'available': available}}
            self.meta.update(metadata)

            data[self.defaultgroup]['hosts'].append(hostname)

            if available == "1":
                data['Available_Zabbix_Agents']['hosts'].append(hostname)
